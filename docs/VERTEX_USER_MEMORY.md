...
8. En shadow mode, revoir explicitement les mémoires `PENDING`, puis les mémoires approuvées affichées comme `APPROUVÉE · SYNCHRO EN ATTENTE` tant que `pendingOperation=UPSERT` et `write_disabled` restent présents.
...
11. Activer `VERTEX_MEMORY_READ_ENABLED=true` pour un compte test. SCRIBE reçoit au plus 8 faits locaux `ACTIVE` déjà convergés (`vertexMemoryName`, `syncedAt`, aucun `pendingOperation` ni `lastSyncError`), ordonnés par pertinence Vertex quand disponible, dans un bloc secondaire de 5 000 caractères maximum. La question libre du client n’est jamais envoyée à Vertex : la recherche utilise une requête générique non personnelle de continuité.
...
Le worker ne scanne que les versions `SEALED` sans job dont `sealedAt >= now - MEMORY_RECOVERY_LOOKBACK_MS`, avec `MEMORY_RECOVERY_LIMIT`. Il sert uniquement à récupérer un enqueue récent manqué, jamais à rejouer l’historique. Après les jobs de lecture, et seulement si l’écriture Vertex est active et que MEMORY est prêt, il converge au plus `MEMORY_PENDING_MUTATION_LIMIT` intentions persistées (`UPSERT`, `DELETE`, `SUPERSEDE`) dans l’ordre de leur ancienneté. Il renouvelle le heartbeat pendant tout le traitement, à un tiers au plus de `MEMORY_JOB_STALE_MS`, puis l’arrête dans un `finally`. Il reprend les jobs `RUNNING` dont heartbeat est expiré avec une mise à jour atomique : un seul worker peut gagner le claim. Les erreurs temporaires (`RESOURCE_EXHAUSTED`, `DEADLINE_EXCEEDED`, `UNAVAILABLE`) reçoivent un backoff exponentiel. Les erreurs d’autorisation, credentials, parent, contenu ou argument sont terminales (`CANCELLED`) et ne sont jamais reprises automatiquement. Un ADMIN peut relancer manuellement un job `FAILED` ou `CANCELLED` depuis le client concerné.

## Conflits, validation et assainissement

L’extraction peut créer des candidats `PENDING`. L’ADMIN peut approuver, corriger, rejeter, supprimer ou resynchroniser un fait. Chaque mutation enregistre d’abord une intention durable `UPSERT`, `DELETE` ou `SUPERSEDE`, puis appelle Vertex, puis finalise PostgreSQL. Avant chaque `UPDATE` ou `DELETE` nominatif, Lumira relit la mémoire distante, vérifie le parent et le scope exact attendu, et refuse toute divergence avant le RPC mutateur. Si Vertex réussit alors que PostgreSQL échoue, l’intention et la référence Vertex restent visibles et une resynchronisation converge sans faux succès. `NOT_FOUND` est idempotent pour DELETE.
...
Après avoir configuré deux comptes techniques vides et distincts, l’ADMIN lance `POST /api/expert/memories/diagnostic`. Le diagnostic refuse tout ID correspondant à un vrai `User`, vérifie les scopes A, B et la variante de casse de A vides, crée dans A, tente une mise à jour de A avec B comme scope attendu et exige son rejet avant RPC, vérifie que A existe toujours, puis vérifie l’absence depuis B et la variante de casse. Il supprime dans un `finally` et re-liste A pour confirmer le nettoyage. Il ne crée aucune ligne locale durable. N’utilisez jamais un compte client réel.

## Purge et rollback

Une purge client détecte toute trace locale (`UserMemory`, mutation en attente ou `MemorySyncJob`), y compris une création Vertex réussie dont la référence locale n’a pas encore été écrite. Pour une telle trace, elle exige parent et credentials valides indépendamment des flags fonctionnels, liste tout le scope Vertex, le supprime, puis le re-liste avant toute suppression S3 ou PostgreSQL. Chaque suppression nominative relit et vérifie le scope utilisateur exact avant son RPC ; `NOT_FOUND` est un succès idempotent. Un compte ancien sans trace mémoire reste supprimable. Une erreur Vertex laisse S3 et PostgreSQL intacts.
