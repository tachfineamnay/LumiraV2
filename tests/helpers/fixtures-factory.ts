...
export function createTestSpiritualPath(
  userId: string,
  overrides: Partial<TestSpiritualPath> = {},
): TestSpiritualPath {
...
export function createTestSubscription(
  ...
  return {
    id: `sub-${Date.now()}`,
    userId,
    stripeSubscriptionId: `pi_test_${Date.now()}`,
...
export function createTestDream(userId: string, overrides: Partial<TestDream> = {}): TestDream {
  return {
    ...
    interpretation: {
      summary:
        'Ce rêve symbolise un retour aux sources et une reconnexion avec la nature profonde de votre être.',
