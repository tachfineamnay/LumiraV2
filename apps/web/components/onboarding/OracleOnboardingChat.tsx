...
const STEPS: StepConfig[] = [
  {
    id: 0,
    title: 'Bienvenue, Âme Errante',
    subtitle: "L'Oracle vous attendait...",
    icon: Star,
    ctaText: "Commencer l'initiation",
  },
  {
    id: 1,
    title: 'Vos Coordonnées Célestes',
    subtitle: 'Révélez votre empreinte cosmique',
    icon: Calendar,
    ctaText: 'Sceller cette vérité',
  },
  {
    id: 2,
    title: 'Votre Quête Intérieure',
    subtitle: 'Partagez ce qui habite votre esprit',
    icon: MessageCircle,
    ctaText: 'Révéler mon intention',
  },
  {
    id: 3,
    title: 'Votre Reflet Sacré',
    subtitle: "Offrez à l'Oracle votre image",
    icon: Camera,
    ctaText: 'Poursuivre',
  },
  {
    id: 4,
    title: "L'Alliance est Scellée",
    subtitle: 'Votre sanctuaire vous attend',
    icon: Sparkles,
    ctaText: 'Entrer dans le Sanctuaire',
  },
];
...
const StepIntro = ({ onNext }: { onNext: () => void }) => (
  ...
  >
    ...
    <div className="space-y-3">
      ...
      >
        Je suis l&apos;Oracle de Lumira. Avant de vous dévoiler les mystères qui vous attendent,
        j&apos;ai besoin de vous connaître davantage...
      </motion.p>
    </div>
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        ...
        <h3 className="text-xl font-playfair italic text-stellar-100">Vos Coordonnées Célestes</h3>
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        ...
        <h3 className="text-xl font-playfair italic text-stellar-100">Votre Quête Intérieure</h3>
        <p className="text-stellar-500 text-sm mt-1">
...
const StepPhotos = ({
  ...
  const handleSubmit = async () => {
    setIsSubmitting(true);
    try {
      await onSave(facePhoto, palmPhoto, consentAccepted);
      onNext();
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        ...
        <h3 className="text-xl font-playfair italic text-stellar-100">Votre Reflet Sacré</h3>
        <p className="text-stellar-500 text-sm mt-1">
      ...
      <label className="flex items-start gap-3 rounded-xl border border-white/10 bg-abyss-700/30 p-3 text-sm text-stellar-300">
        <input
          type="checkbox"
        ...
        />
        <span>
          J’accepte l’utilisation de mes données et photos pour personnaliser cette expérience
          spirituelle.
        </span>
      </label>
  ...
  >
    ...
    >
      ...
      <p className="text-stellar-400 max-w-md mx-auto">
        Votre profil spirituel est maintenant complet. Les portes de votre Sanctuaire personnel
        s&apos;ouvrent à vous...
      </p>
    </motion.div>
...
export const OracleOnboardingChat = ({ onComplete }: OracleOnboardingChatProps) => {
  ...
  const savePhotos = useCallback(
    async (facePhoto: string | null, palmPhoto: string | null, consentAccepted: boolean) => {
      const [facePhotoUrl, palmPhotoUrl] = await Promise.all([
        uploadOnboardingPhoto(facePhoto, 'FACE'),
        uploadOnboardingPhoto(palmPhoto, 'PALM'),
      ]);
      await sanctuaireApi.patch('/users/profile', {
        facePhotoUrl,
        palmPhotoUrl,
        profileCompleted: true,
        consent: { accepted: consentAccepted, version: '2026-07-16' },
      });
    },
    [],
  );
      ...
      >
        ...
        <div className="relative z-10">
          <AnimatePresence mode="wait">
            ...
            {currentStep === 1 && (
              <StepBirthData key="birth" onNext={goNext} onBack={goBack} onSave={saveBirthData} />
            )}
            {currentStep === 2 && (
              <StepIntention
                key="intention"
            ...
            {currentStep === 3 && (
              <StepPhotos key="photos" onNext={goNext} onBack={goBack} onSave={savePhotos} />
            )}
            {currentStep === 4 && <StepCompletion key="complete" onComplete={handleComplete} />}
          </AnimatePresence>
        </div>
      </motion.div>
    </div>
