...
function toPersistableDraft(
  data: Partial<HolisticDiagnosticData>,
): Partial<HolisticDiagnosticData> {
  return {
    ...data,
    // Base64 previews stay in component memory only. A completed upload is
    // represented by the private S3 reference returned by the API.
...
export const HolisticWizard = ({
  ...
  const [remoteDraftLoaded, setRemoteDraftLoaded] = useState(false);
  ...
  // The server draft is authoritative across devices; localStorage only keeps
  // an offline fallback and deliberately excludes transient Base64 previews.
  const formData = watch();
  useEffect(() => {
    let active = true;
    const loadRemoteDraft = async () => {
      try {
        const response = await sanctuaireApi.get('/users/onboarding');
        ...
        setStep(Math.min(Math.max(remote.currentStep || 0, 0), STEPS.length - 1));
      } catch {
        // Offline or first visit: the local fallback remains available.
      } finally {
        if (active) setRemoteDraftLoaded(true);
      }
    };
    loadRemoteDraft();
    return () => {
    ...
    // This draft is tied to the authenticated user; reload only on identity change.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [reset, userEmail]);

  useEffect(() => {
    if (!remoteDraftLoaded) return;
    const persisted = toPersistableDraft(formData);
    localStorage.setItem('holistic_wizard_draft', JSON.stringify(persisted));
    ...
    saveTimerRef.current = setTimeout(() => {
      sanctuaireApi
        ...
        .catch(() => {
          // The local fallback will be retried on the next interaction.
        });
    }, 700);
  ...
  }, [formData, remoteDraftLoaded, step]);
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="hidden sm:block">
                  <span className="text-sm font-playfair italic text-stellar-100">
                    Oracle Lumira
                  </span>
                  <span className="block text-[9px] text-stellar-500 uppercase tracking-wider">
                    Diagnostic
                  </span>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="flex justify-center gap-3">
                    {['Left', 'Right'].map((side) => (
                      ...
                      >
                        <span
                          className={`block text-lg font-medium mb-0.5 ${watch('strongSide') === side ? 'text-horizon-300' : 'text-stellar-400'}`}
                        >
                          {side === 'Left' ? '🤚 Gauche' : '✋ Droite'}
                        </span>
                        <span className="text-[10px] uppercase tracking-wider text-stellar-500">
                          Côté Fort
                        </span>
                      </button>
                    ))}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs uppercase tracking-wider text-stellar-500 font-medium mb-1.5 block">
                        Zone fragile
                      </label>
                      <input
                        {...register('weakZone')}
                      ...
                      />
                      {errors.weakZone && (
                        <span className="text-rose-400 text-xs">{errors.weakZone.message}</span>
                      )}
                    </div>
                    <div>
                      <label className="text-xs uppercase tracking-wider text-stellar-500 font-medium mb-1.5 block">
                      ...
                      />
                      {errors.strongZone && (
                        <span className="text-rose-400 text-xs">{errors.strongZone.message}</span>
                      )}
                    </div>
                  </div>
                </motion.div>
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <div className="space-y-2">
                    {(Object.keys(DELIVERY_STYLES) as Array<keyof typeof DELIVERY_STYLES>).map(
                      (style) => (
                        <button
                          key={style}
                          type="button"
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <h4
                              className={`font-medium text-sm ${watch('deliveryStyle') === style ? 'text-horizon-300' : 'text-stellar-200'}`}
                            >
                              {DELIVERY_STYLES[style].title}
                              <span className="text-stellar-500 font-normal ml-1.5">
                                — {DELIVERY_STYLES[style].subtitle}
                              </span>
                            </h4>
                            <p className="text-xs text-stellar-500 truncate">
                              {DELIVERY_STYLES[style].description}
                            </p>
                          </div>
                          <div
                          ...
                          >
                            {watch('deliveryStyle') === style && (
                              <Check className="w-full h-full text-abyss-900 p-0.5" />
                            )}
                          </div>
                        </button>
                      ),
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        />
                        {errors.birthDate && (
                          <span className="text-rose-400 text-xs">{errors.birthDate.message}</span>
                        )}
                      </div>
                      <div>
                        <label className="text-xs uppercase tracking-wider text-stellar-500 font-medium mb-1.5 block">
                    ...
                    <div>
                      ...
                      />
                      {errors.birthPlace && (
                        <span className="text-rose-400 text-xs">{errors.birthPlace.message}</span>
                      )}
                    </div>

                    {/* Photos - Compact on mobile */}
                    <div className="pt-2">
                      <p className="text-xs text-stellar-500 mb-3 text-center">
                        Photos optionnelles pour une lecture approfondie
                      </p>
                      <div className="grid grid-cols-2 gap-3">
                        <SmartPhotoUploader
                          label="Visage"
                ...
                >
                  ...
                  <label className="flex items-start gap-3 p-4 rounded-xl bg-abyss-700/30 border border-white/5 cursor-pointer hover:bg-abyss-700/50 transition-colors">
                    ...
                    <span className="text-sm text-stellar-300">
                      J'accepte que mes données soient utilisées pour personnaliser mon expérience
                      spirituelle avec Oracle Lumira.
                    </span>
                  </label>
                  {errors.gdprConsent && (
                    <span className="text-rose-400 text-xs block text-center">
