...
export function OrderCard({
  ...
  const pipeline = (
    order.generatedContent as unknown as {
  ...
  const intakeStatus = order.readingIntake?.status;
  const isWaitingForClient = order.intakeRequired === true && intakeStatus !== 'SEALED';
  ...
  const claim = (event: React.MouseEvent) => {
    ...
    if (!isClaiming && !isWaitingForClient) void onClaim?.(order.id);
  };
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-desk-muted">
        ...
        {isAssignedToMe && (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-emerald-700">
            <UserCheck className="h-3 w-3" />À moi
          </span>
        )}
        {isAssignedToOther && (
          <span className="inline-flex items-center gap-1 rounded-full bg-orange-500/10 px-2 py-0.5 text-orange-700">
            <UserCheck className="h-3 w-3" />
            {review.assignedName || 'Prise'}
          </span>
        )}
      </div>
      ...
      {isWaitingForClient && (
        <div className="mt-3 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs leading-5 text-amber-800">
          <p className="font-semibold">Dossier client à confirmer</p>
          <p className="mt-0.5">
            {intakeStatus === 'DRAFT'
              ? 'Le client peut encore relire et modifier son brouillon avant de le sceller.'
              : 'Le client doit compléter puis sceller son dossier avant la prise en charge.'}
          </p>
        </div>
      )}
      ...
      {columnId === 'paid' && !review.assignedBy && onClaim ? (
        <button
          type="button"
          onClick={claim}
          disabled={isClaiming || isWaitingForClient}
          className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-amber-500/15 text-sm font-semibold text-amber-700 hover:bg-amber-500/25 disabled:cursor-not-allowed disabled:opacity-55"
        >
          {isClaiming ? <Loader2 className="h-4 w-4 animate-spin" /> : <Hand className="h-4 w-4" />}
          {isWaitingForClient
        ...
        <button
          ...
          className={`mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg text-sm font-semibold ${action.primary ? 'bg-amber-500 text-slate-950 hover:bg-amber-400' : 'border border-desk-border text-desk-muted hover:bg-desk-hover hover:text-desk-text'}`}
        >
...
function getAction(status: string, assigned: boolean) {
  if (status === 'PAID')
    return { label: assigned ? 'Préparer la lecture' : 'Ouvrir', primary: assigned };
...
function QualityBadge({ status }: { status?: 'PASS' | 'WARNING' | 'BLOCKED' }) {
  if (status === 'PASS')
    return (
      <span title="Qualité validée" className="text-emerald-600">
        <CheckCircle2 className="h-5 w-5" />
      </span>
    );
  if (status === 'WARNING')
    return (
      <span title="Avertissements" className="text-amber-600">
        <AlertTriangle className="h-5 w-5" />
      </span>
    );
  if (status === 'BLOCKED')
    return (
      <span title="Corrections requises" className="text-red-600">
