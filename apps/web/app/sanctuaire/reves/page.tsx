...
export default function DreamJournalPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-3 sm:px-6 py-4 sm:py-8 overflow-x-hidden">
      {/* Header */}
      <div className="flex items-center justify-between gap-3 mb-8">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-playfair italic text-white flex items-center gap-2 sm:gap-3">
            <Moon className="w-6 h-6 sm:w-7 sm:h-7 text-serenity-400 flex-shrink-0" />
            <span className="truncate">Journal des Rêves</span>
          </h1>
          <p className="text-stellar-500 text-sm mt-1">
            {dreams.length} rêve{dreams.length !== 1 ? 's' : ''} enregistré
            {dreams.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <Link
          ...
          className="flex items-center gap-2 px-4 sm:px-5 py-3 min-h-[44px] rounded-xl bg-gradient-to-r from-serenity-500 to-serenity-600 text-white font-semibold hover:from-serenity-400 hover:to-serenity-500 transition-all hover:shadow-[0_0_20px_rgba(94,234,212,0.3)] flex-shrink-0"
        >
      ...
      {dreams.length === 0 && (
        ...
        >
          ...
          <h2 className="text-xl font-playfair italic text-white mb-3">Votre journal est vierge</h2>
          <p className="text-stellar-500 max-w-sm mx-auto mb-8 text-sm leading-relaxed">
            Commencez à consigner vos rêves et Lumira vous révélera les messages cachés de votre
            inconscient.
          </p>
          <Link
            href="/sanctuaire/reves/nouveau"
