...
import {
  ...
  ArchetypeCard,
  SynthesisContent,
} from '../../../components/sanctuary/synthesis';
      ...
      >
        ...
        <div>
          ...
          <p className="text-slate-400 leading-relaxed max-w-md mx-auto">
            Votre essence spirituelle est en cours de révélation par l&apos;Oracle. Cette synthèse
            sera disponible après la génération de votre lecture personnalisée.
          </p>
        </div>
...
function AstroCard({
  ...
  return (
    ...
    >
      ...
      <span className="text-xs uppercase tracking-wider text-slate-500 block mb-1">{label}</span>
      <span className="text-sm font-medium text-white">{value || 'Non renseigné'}</span>
    </motion.div>
  );
}
...
export default function SynthesisPage() {
  ...
  const hasAccess =
    highestLevel >= 1 ||
    hasCapability('synthesis_dashboard') ||
    hasCapability('sanctuaire.sphere.synthesis');
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8 overflow-x-hidden">
      {/* Hero Header */}
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            <button
              onClick={refetch}
              className="flex items-center gap-2 px-3 sm:px-4 py-2.5 min-h-[44px] rounded-xl bg-slate-800/50 border border-slate-700 text-slate-400 hover:bg-slate-700/50 hover:text-slate-300 transition-all text-sm"
            >
              <RefreshCw className="w-4 h-4" />
            ...
            <Link
              href="/sanctuaire"
              className="flex items-center gap-2 px-4 sm:px-5 py-2.5 min-h-[44px] rounded-xl bg-slate-800/50 border border-slate-700 text-slate-300 hover:border-amber-500/30 transition-all"
            >
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Card A: The Mirror */}
          <SoulMirror photoUrl={photoUrl} userName={userName} archetype={archetype} />
      ...
      {categories.length > 0 && categories.some((c) => c.insight) && (
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-2xl font-playfair italic text-white">Vos Insights</h2>
            {newInsightsCount > 0 && (
        ...
        >
          <GlassCard className="p-6 text-center max-w-2xl mx-auto">
            ...
            <p className="text-slate-500 text-sm">
              Notre agent IA analyse votre parcours spirituel et prépare des insights personnalisés
              pour chaque dimension de votre vie.
            </p>
          </GlassCard>
        </motion.div>
      )}
      ...
      {/* Insight Modal */}
      <InsightModal data={selectedInsight} isOpen={isModalOpen} onClose={handleCloseModal} />
    </div>
  );
}
