  ...
  return jest.fn().mockImplementation(() => ({
    ...
    checkout: {
      sessions: {
        create: jest
          .fn()
          .mockResolvedValue({ id: 'cs_test_123', url: 'https://checkout.stripe.com/pay/test' }),
      },
    },
    subscriptions: {
...
describe('SubscriptionsService', () => {
  ...
  describe('getStatus', () => {
    it('should return subscription data when exists', async () => {
      const mockSub = {
        ...
        stripeSubscriptionId: 'pi_lifetime_123',
      };
      ...
      expect(result.subscription).toEqual(
        expect.objectContaining({ ...mockSub, isLifetimeAccess: true, cancellable: false }),
      );
    });
  ...
  describe('cancel', () => {
    ...
    it('rejects cancellation for a one-time lifetime access', async () => {
      prisma.subscription.findUnique.mockResolvedValue({
        status: 'ACTIVE',
        cancelAtPeriodEnd: false,
        stripeSubscriptionId: 'pi_lifetime_123',
