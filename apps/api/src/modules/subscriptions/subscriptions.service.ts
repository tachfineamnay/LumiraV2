...
export class SubscriptionsService {
  ...
  /**
   * Creates a one-time Checkout Session for lifetime Sanctuaire access.
   */
  ...
  async getStatus(userId: string) {
    ...
    const isRecurring = subscription.stripeSubscriptionId.startsWith('sub_');
    return {
      hasSubscription: true,
      subscription: {
        ...subscription,
        isLifetimeAccess: !isRecurring,
        cancellable: isRecurring,
      },
    };
  }
  ...
  async cancel(userId: string) {
    ...
    if (!sub.stripeSubscriptionId.startsWith('sub_')) {
      throw new BadRequestException(
        'Cet accès est acquis à vie et ne comporte aucun renouvellement à annuler.',
      );
    }
    ...
    this.logger.log(
      `[Cancel] Subscription ${sub.stripeSubscriptionId} scheduled for cancellation.`,
    );
    return updated;
  }
  ...
  async resume(userId: string) {
    ...
    if (!sub.stripeSubscriptionId.startsWith('sub_')) {
      throw new BadRequestException(
        'Cet accès est acquis à vie et ne comporte aucun renouvellement à reprendre.',
      );
    }
