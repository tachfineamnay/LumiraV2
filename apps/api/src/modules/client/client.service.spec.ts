...
describe('ClientService', () => {
  ...
  describe('completeStep', () => {
    ...
    it('should throw NotFoundException for step not belonging to user', async () => {
      ...
      await expect(service.completeStep('user-1', 'step-wrong')).rejects.toThrow(NotFoundException);
    });

    it('should verify step ownership via spiritual path userId', async () => {
      prisma.pathStep.findFirst.mockResolvedValue(null);

      await service.completeStep('user-1', 'step-1').catch(() => undefined);
  ...
  describe('getChatQuota', () => {
    ...
    it('should throw NotFoundException for unknown user', async () => {
      ...
      await expect(service.getChatQuota('nonexistent')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('chatWithOracle', () => {
    ...
    it('should throw ForbiddenException when no completed reading (NO_ACCESS)', async () => {
      ...
      await expect(service.chatWithOracle('user-1', 'Question')).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe('getClientProfile', () => {
    ...
    it('should throw NotFoundException for unknown user', async () => {
      ...
      await expect(service.getClientProfile('nonexistent')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('getReadingContent', () => {
    ...
    it('should throw NotFoundException for non-existent or non-owned order', async () => {
      ...
      await expect(service.getReadingContent('user-1', 'wrong-order')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
