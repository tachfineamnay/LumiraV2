...
export class ClientService {
  ...
  async getClientProfile(userId: string) {
    ...
    if (latestReading?.generatedContent) {
      const content = latestReading.generatedContent as ReadingContent;
  ...
  async getCompletedReadings(userId: string) {
    ...
    const completedReadings = completedOrders.map((order) => {
      ...
      const archetype = (content?.archetype as string) || (synthesis?.archetype as string) || null;
      ...
      return {
        ...
        archetype,
        title: archetype ? `Lecture d'Âme - ${archetype}` : `Lecture d'Âme #${order.orderNumber}`,
  ...
  async getReadingContent(userId: string, orderId: string) {
    ...
    const content = order.generatedContent as ReadingContent | null;
  ...
  async chatWithOracle(userId: string, message: string, sessionId?: string) {
    ...
    if (
      ...
      quotaStatus.messagesRemaining === 0 &&
    ...
    try {
      ...
      const response = await this.contextDispatcher.dispatchChatRequest(userId, message, sessionId);
