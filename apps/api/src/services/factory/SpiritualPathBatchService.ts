...
import { productLevelFromAmountCents } from '../factory/product-level.util';
...
export class SpiritualPathBatchService {
  ...
  private async processBatch(batchNumber: 2 | 3): Promise<void> {
    ...
    this.logger.log(
      `🗓️ [Batch ${batchNumber}] Processing days ${startDay}-${endDay} (cutoff: ${cutoffDate.toISOString()})`,
    );
  ...
  private async generateBatchForUser(userId: string, batchNumber: 1 | 2 | 3): Promise<void> {
    const startDay = (batchNumber - 1) * 10 + 1;
    const endDay = batchNumber * 10;
    this.logger.log(
      `🔮 Generating batch ${batchNumber} (days ${startDay}-${endDay}) for user ${userId.substring(0, 8)}...`,
    );
    ...
    const latestOrder = await this.prisma.order.findFirst({
      where: { userId, status: { in: ['PAID', 'PROCESSING', 'COMPLETED'] } },
      orderBy: { createdAt: 'desc' },
      select: { id: true, amount: true },
    });

    const routing = latestOrder
      ? {
          orderId: latestOrder.id,
          productLevel: productLevelFromAmountCents(latestOrder.amount),
        }
      : undefined;

    const pastDreams =
      batchNumber >= 2
        ? await this.prisma.dream
            .findMany({
              where: {
                userId,
    ...
    this.logger.log(
      `✅ Batch ${batchNumber}: ${timelineDays.length} steps persisted for user ${userId.substring(0, 8)}`,
    );
  }
