...
export class ContextDispatcher {
  ...
  async dispatchChatRequest(
    ...
    const reply = await this.vertexOracle.chatWithUser(
      ...
      lastOrder
        ? {
            ...
            productLevel: productLevelFromAmountCents(lastOrder.amount),
          }
        : undefined,
    );
    ...
    await this.saveConversation(userId, newSessionId, message, reply, lastOrder?.id);
  ...
  async finalizeOrder(
    ...
    const synthesis =
      existingContent?.synthesis || (await this.extractSynthesisFromContent(content));
    ...
    const pdfData: ReadingPdfData = {
      ...
      conclusion:
        existingContent?.pdf_content?.conclusion || this.extractSection(content, 'conclusion'),
      birthData: {
  ...
  private async upsertAkashicRecord(
    ...
    try {
      ...
      const now = new Date();
      const id =
        existing?.id || `akashic_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
  ...
  private extractSections(
    content: string,
  ): Array<{ domain: string; title: string; content: string }> {
